...
export function StepRole({ control }: Props) {
  return (
    <Controller
      control={control}
      name="role"
      render={({ field }) => (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-3">
          <p className="text-[11px] font-bold text-text-muted uppercase tracking-wider">Selecciona el tipo de perfil</p>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? cn(config.badge, "ring-2 ring-offset-2 ring-current shadow-md border-transparent")
                ...
                >
                  ...
                  <div>
                    <p className={cn("text-sm font-semibold tracking-tight", isActive ? "text-current" : "text-text-primary dark:text-white")}>
                      {config.label}
                    </p>
                    <p className="text-[11.5px] text-text-secondary mt-1 leading-snug">{config.desc}</p>
                  </div>
                </button>
              );
            })}
